...
bool medMeshToolsPlugin::initialize()
{
    ...
    if ( !smoothMeshProcess::registered() )
        dtkWarn() << "Unable to register smoothMeshProcess";

    if ( !decimateMeshProcess::registered() )
        dtkWarn() << "Unable to register decimateMeshProcess";
