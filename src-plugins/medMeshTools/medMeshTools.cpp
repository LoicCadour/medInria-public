...
qDebug()<<"transformFilter->Update();";
    polydata->DeepCopy(transformFilter->GetOutput());

    vtkMetaSurfaceMesh * smesh = vtkMetaSurfaceMesh::New();qDebug()<<"AVANT smesh->SetDataSet(polydata);";
    smesh->SetDataSet(polydata);
    qDebug()<<"APRES smesh->SetDataSet(polydata);";
    contour->Delete();
    contourTrian->Delete();
    if (contourDecimated) contourDecimated->Delete();
    if (contourSmoothed) contourSmoothed->Delete();
qDebug()<<"Avant set patient info";
	output->copyMetaDataFrom(input);
qDebug()<<"output->setData(smesh)";
...
void medMeshTools::setInput ( medAbstractData *data )
{
    if ( !data )
        return;
