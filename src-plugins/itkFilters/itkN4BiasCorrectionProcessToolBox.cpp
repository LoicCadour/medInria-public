...
public:
    ...
    dtkSmartPointer <itkN4BiasCorrectionProcess> process;
...
void itkN4BiasCorrectionProcessToolBox::run()
{
    ...
    d->process = new itkN4BiasCorrectionProcess();
    ...
    d->process->setParameter((double)d->saveBiasCheckBox->isChecked(), 6);

    std::vector<int> nbOfIterations = this->extractValue(d->nbOfIterationsText->text());
    for (int i = 0; i<nbOfIterations.size();i++)
...
std::vector<int> itkN4BiasCorrectionProcessToolBox::extractValue(QString text)
{
    std::vector<int> out;
