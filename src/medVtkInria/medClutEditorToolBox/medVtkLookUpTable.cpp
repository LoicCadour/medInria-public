...
const vtkIdType medVtkLookUpTable::BELOW_RANGE_COLOR_INDEX  = 0;
const vtkIdType medVtkLookUpTable::ABOVE_RANGE_COLOR_INDEX  = 1;
const vtkIdType medVtkLookUpTable::NAN_COLOR_INDEX          = 2;
const vtkIdType medVtkLookUpTable::NUMBER_OF_SPECIAL_COLORS = NAN_COLOR_INDEX+1;

vtkStandardNewMacro(medVtkLookUpTable);

// Construct with range=(0,1); and hsv ranges set up for rainbow color table
// (from red to blue).
medVtkLookUpTable::medVtkLookUpTable(int sze, int ext)
{
...
//----------------------------------------------------------------------------
medVtkLookUpTable::~medVtkLookUpTable()
{
}
...
int medVtkLookUpTable::IsOpaque()
{
...
// range value.
void medVtkLookUpTable::SetTableRange(double r[2])
{
...
// Scalar values greater than maximum range value are clamped to maximum
// range value.
void medVtkLookUpTable::SetTableRange(double rmin, double rmax)
{
...
void medVtkLookUpTable::SetScale(int scale)
{
  if (this->Scale == scale)
    {
    return;
...
int medVtkLookUpTable::Allocate(int sz, int ext)
{
...
void medVtkLookUpTable::ForceBuild()
{
  int i;
...
void medVtkLookUpTable::GetColor(double v, double rgb[3])
{
...
double medVtkLookUpTable::GetOpacity(double v)
{
  unsigned char *rgb8 = this->MapValue(v);
...
inline vtkIdType vtkLinearIndexLookupMain(double v, const TableParameters & p)
{
  ...
  if (v < p.Range[0])
    {
    dIndex = p.MaxIndex + medVtkLookUpTable::BELOW_RANGE_COLOR_INDEX + 1.5;
    }
  else if (v > p.Range[1])
    {
    dIndex = p.MaxIndex + medVtkLookUpTable::ABOVE_RANGE_COLOR_INDEX + 1.5;
    }
  else
    {
  ...
  double v, unsigned char *table, const TableParameters & p)
{
  if (vtkMath::IsNan(v))
    ...
    return table + 4*(maxIndex + medVtkLookUpTable::NAN_COLOR_INDEX);
    }
...
//----------------------------------------------------------------------------
void medVtkLookUpTable::GetLogRange(const double range[2], double log_range[2])
{
  vtkLookupTableLogRange(range, log_range);
}

//----------------------------------------------------------------------------
double medVtkLookUpTable::ApplyLogScale(double v, const double range[2],
...
vtkIdType medVtkLookUpTable::GetIndex(double v)
{
  TableParameters p;
...
void medVtkLookUpTable::SetTable(vtkUnsignedCharArray *table)
{
...
//----------------------------------------------------------------------------
void medVtkLookUpTable::GetColorAsUnsignedChars(const double colorIn[4],
...
//----------------------------------------------------------------------------
unsigned char* medVtkLookUpTable::GetNanColorAsUnsignedChars()
{
...
unsigned char* medVtkLookUpTable::MapValue(double v)
{
    std::cout<<"medVtkLookUpTable::MapValue"<<std::endl;
...
//----------------------------------------------------------------------------
template<class T>
void vtkLookupTableMapData(medVtkLookUpTable *self, T *input,
                           ...
                           int inIncr, int outFormat, TableParameters & p)
{
  ...
  vtkIdType neededSize = (numberOfColors + medVtkLookUpTable::NUMBER_OF_SPECIAL_COLORS) *
    lookupTable->GetNumberOfComponents();
  if (lookupTable->GetSize() < neededSize)
    {
    lookupTable->Resize(numberOfColors + medVtkLookUpTable::NUMBER_OF_SPECIAL_COLORS);
    }
  ...
  // Below range color
  unsigned char *tptr = table + 4*(numberOfColors + medVtkLookUpTable::BELOW_RANGE_COLOR_INDEX);
  unsigned char color[4];
  if (self->GetUseBelowRangeColor())
    {
    medVtkLookUpTable::GetColorAsUnsignedChars(self->GetBelowRangeColor(), color);
    tptr[0] = color[0];
    tptr[1] = color[1];
    tptr[2] = color[2];
    tptr[3] = color[3];
  ...
  // Above range color
  tptr = table + 4*(numberOfColors + medVtkLookUpTable::ABOVE_RANGE_COLOR_INDEX);
  if (self->GetUseAboveRangeColor())
    {
    medVtkLookUpTable::GetColorAsUnsignedChars(self->GetAboveRangeColor(), color);
    tptr[0] = color[0];
    tptr[1] = color[1];
    tptr[2] = color[2];
    tptr[3] = color[3];
  ...
  // Always use NanColor
  medVtkLookUpTable::GetColorAsUnsignedChars(self->GetNanColor(), color);
  tptr = table + 4*(numberOfColors + medVtkLookUpTable::NAN_COLOR_INDEX);
  tptr[0] = color[0];
  tptr[1] = color[1];
  tptr[2] = color[2];
  tptr[3] = color[3];
...
void vtkLookupTableIndexedMapData(
  medVtkLookUpTable* self, T* input, unsigned char* output, int length,
  int inIncr, int outFormat )
{
  ...
  medVtkLookUpTable::GetColorAsUnsignedChars(self->GetNanColor(), nanColor);
...
//----------------------------------------------------------------------------
void medVtkLookUpTable::MapScalarsThroughTable2(void *input,
...
void medVtkLookUpTable::SetNumberOfTableValues(vtkIdType number)
{
...
void medVtkLookUpTable::SetTableValue(vtkIdType indx, double rgba[4])
{
  ...
  //std::cout<<"rgba[0] : "<<rgba[0]<<" _rgba[0] : "<<_rgba[0]<<" rgba[1] : "<<rgba[1]<<" _rgba[1] : "<<_rgba[1]<<std::endl;
  this->InsertTime.Modified();
  this->Modified();
}
...
void medVtkLookUpTable::SetTableValue(vtkIdType indx, double r, double g, double b,
                                   double a)
{
  double rgba[4];
...
void medVtkLookUpTable::GetTableValue(vtkIdType indx, double rgba[4])
{
  unsigned char *_rgba;
...
// Return a rgba color value for the given index into the lookup table. Color
// components are expressed as [0,1] double values.
double *medVtkLookUpTable::GetTableValue(vtkIdType indx)
{
...
//----------------------------------------------------------------------------
void medVtkLookUpTable::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void medVtkLookUpTable::DeepCopy(vtkScalarsToColors *obj)
{
  if (!obj)
    {
    return;
  ...
  medVtkLookUpTable *lut = medVtkLookUpTable::SafeDownCast(obj);

  if (!lut)
    {
    vtkErrorMacro("Cannot DeepCopy a " << obj->GetClassName()
                  << " into a medVtkLookUpTable.");
    return;
    }
...
//----------------------------------------------------------------------------
vtkIdType medVtkLookUpTable::GetNumberOfAvailableColors()
{
  return this->Table->GetNumberOfTuples();
}

//----------------------------------------------------------------------------
void medVtkLookUpTable::GetIndexedColor(vtkIdType idx, double rgba[4])
{
